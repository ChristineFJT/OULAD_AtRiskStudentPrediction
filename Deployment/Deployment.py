    ...
    if uploaded_file is not None:
        ...
        if not set(required_columns).issubset(input_data.columns):
            ...
            if (input_data[numeric_features] < 0).any().any():
                ...
                filter_highest_education = st.sidebar.multiselect("Highest Education", 
                                                                  options=decoded_input_data["Highest Education"].unique(), 
                                                                  default=decoded_input_data["Highest Education"].unique())

                # IMD Band filter
                filter_imd_band = st.sidebar.multiselect("IMD Band", 
                                                         options=decoded_input_data["IMD Band"].unique(), 
                                                         default=decoded_input_data["IMD Band"].unique())
                
                # Conditional slider for 'Number of Previous Attempts'
                num_of_prev_attempts_min = int(decoded_input_data["Number of Previous Attempts"].min())
                num_of_prev_attempts_max = int(decoded_input_data["Number of Previous Attempts"].max())

                if num_of_prev_attempts_min != num_of_prev_attempts_max:
                    filter_num_of_prev_attempts = st.sidebar.slider("Number of Previous Attempts", 
                                                                    min_value=num_of_prev_attempts_min, 
                                                                    max_value=num_of_prev_attempts_max, 
                                                                    value=(num_of_prev_attempts_min, num_of_prev_attempts_max))
                else:
                    st.sidebar.write(f"Number of Previous Attempts: {num_of_prev_attempts_min}")
                    filter_num_of_prev_attempts = (num_of_prev_attempts_min, num_of_prev_attempts_max)

                # Conditional slider for 'Studied Credits'
                studied_credits_min = int(decoded_input_data["Studied Credits"].min())
                studied_credits_max = int(decoded_input_data["Studied Credits"].max())

                if studied_credits_min != studied_credits_max:
                    filter_studied_credits = st.sidebar.slider(
                        "Studied Credits", 
                        min_value=studied_credits_min, 
                        max_value=studied_credits_max, 
                        value=(studied_credits_min, studied_credits_max)
                    )
                else:
                    st.sidebar.write(f"Studied Credits: {studied_credits_min}")
                    filter_studied_credits = (studied_credits_min, studied_credits_max)

                # Conditional slider for 'Before Course Clicks'
                before_course_click_min = int(decoded_input_data["Before Course Clicks"].min())
                before_course_click_max = int(decoded_input_data["Before Course Clicks"].max())

                if before_course_click_min != before_course_click_max:
                    filter_before_course_click = st.sidebar.slider(
                        "Before Course Clicks", 
                        min_value=before_course_click_min, 
                        max_value=before_course_click_max, 
                        value=(before_course_click_min, before_course_click_max)
                    )
                else:
                    st.sidebar.write(f"Before Course Clicks: {before_course_click_min}")
                    filter_before_course_click = (before_course_click_min, before_course_click_max)

                # Conditional slider for 'After Course Clicks'
                after_course_click_min = int(decoded_input_data["After Course Clicks"].min())
                after_course_click_max = int(decoded_input_data["After Course Clicks"].max())

                if after_course_click_min != after_course_click_max:
                    filter_after_course_click = st.sidebar.slider(
                        "After Course Clicks", 
                        min_value=after_course_click_min, 
                        max_value=after_course_click_max, 
                        value=(after_course_click_min, after_course_click_max)
                    )
                else:
                    st.sidebar.write(f"After Course Clicks: {after_course_click_min}")
                    filter_after_course_click = (after_course_click_min, after_course_click_max)

                # Conditional slider for 'Final Score'
                final_score_min = float(decoded_input_data["Final Score"].min())
                final_score_max = float(decoded_input_data["Final Score"].max())

                if final_score_min != final_score_max:
                    filter_final_score = st.sidebar.slider(
                        "Final Score", 
                        min_value=final_score_min, 
                        max_value=final_score_max, 
                        value=(final_score_min, final_score_max)
                    )
                else:
                    st.sidebar.write(f"Final Score: {final_score_min}")
                    filter_final_score = (final_score_min, final_score_max)
